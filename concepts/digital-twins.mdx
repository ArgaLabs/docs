...
A digital twin is a stateful, behavioural clone of any external tool — Stripe, Slack, Discord, Google Drive, Google Calendar, Box, Notion, Unstructured, or any third-party API — that replicates the endpoints, edge cases, and observable behaviours of the real service.
...
| `dropbox` | Dropbox API v2 | Exposed as a sandbox surface in the UI. Supports storage file operations (list, get, create, update, delete), Dropbox SDK passthrough routes (e.g. `files/list_folder`, `files/upload`), webhook deliveries, and event replay. |
