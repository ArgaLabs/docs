...
A digital twin is a stateful, behavioural clone of any external tool — Stripe, Slack, Discord, Dropbox, Google Drive, Google Calendar, Box, GitHub, Notion, Unstructured, or any third-party API — that replicates the endpoints, edge cases, and observable behaviours of the real service.
...
| `github` | GitHub REST API (`api.github.com`, `github.com`) | Exposed as a sandbox surface in the UI. Intercepts GitHub REST API operations including repos, branches, commits, pull requests, issues, contents, check runs, search, orgs, users, and webhooks. Supports deterministic seeding, bearer/token authentication, OAuth flow emulation, and a browser-facing UI for browsing repository state. |
