...
| `/validate/url` | `POST` | Yes | Yes | **Removed** — use `/validate/url-run` |
...
| `/redteam/start` | `POST` | — | Yes | **Removed** — use [`/validate/agent-run`](#post-start-a-sandbox-run) |
| `/redteam/start-branch` | `POST` | — | Yes | **Removed** — use [`/validate/agent-run`](#post-start-a-sandbox-run) |
...
| `/integrations/{provider}/settings` | `PATCH` | — | Yes | |
| `/validate/{run_id}/env/discord/_frontend/state` | `GET` | Yes | Yes | Discord twin must be active on the run |
| `/validate/{run_id}/env/discord/_frontend/messages` | `POST` | Yes | Yes | Discord twin must be active on the run |
...
| `scenario_id` | `string` | No | UUID of a saved [scenario](/features/custom-scenarios) to pre-seed twins with sample data. When provided without `scenario_prompt`, the scenario's own prompt is used. |
| `twins` | `array of string` | No | Digital twin services to include in the run (e.g. `slack`, `stripe`). When provided, the specified twins are merged with any twins auto-detected from the repository's runtime profile. Only valid twin names from the [twin catalog](#get-list-available-twins) are accepted. |
...
<Warning>The `/validate/url` endpoint has been removed. Use [`/validate/url-run`](#post-start-a-url-run) instead.</Warning>

The `POST /validate/url` endpoint was previously deprecated and has now been removed. Migrate to [`/validate/url-run`](#post-start-a-url-run), which supports both prompted and auto-planned runs.
...
## Discord twin controls

These endpoints proxy requests to the Discord digital twin running inside a sandbox. Use them to inspect twin state and send messages to Discord channels during a validation run.

### `GET` Get Discord twin state

<Note>Accepts both API key and JWT authentication.</Note>

```bash
GET /validate/{run_id}/env/discord/_frontend/state
```

Return the current state of the Discord twin for a validation run, including available channels and the bot user. The run must have a deployed environment with an active Discord twin.
...
```bash
curl "https://api.argalabs.com/validate/a1b2c3d4-e5f6-7890-abcd-ef1234567890/env/discord/_frontend/state" \
  -H "Authorization: Bearer $ARGA_API_KEY"
```
...
| Field | Type | Description |
|-------|------|-------------|
| `channels` | `object` | Map of channel ID to channel details. Each entry includes `id` and `name`. |
| `bot_user` | `object \| null` | The bot user object from the Discord twin, or `null` if the bot user could not be retrieved. |

**Errors**

| Status | Detail |
|--------|--------|
| `400` | `Run has no deployed environment` |
| `400` | `Discord twin not configured` |
| `401` | Invalid or expired token. |
| `404` | `Run not found` |
| `502` | `Failed to reach Discord twin` |

### `POST` Send a message to Discord twin

<Note>Accepts both API key and JWT authentication.</Note>

```bash
POST /validate/{run_id}/env/discord/_frontend/messages
```

Send a message to a channel in the Discord twin. The run must have a deployed environment with an active Discord twin.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `channel_id` | `string` | Yes | ID of the Discord channel to send the message to. Use the [state endpoint](#get-get-discord-twin-state) to discover available channels. |
| `content` | `string` | Yes | Message content to send. Must not be empty after trimming whitespace. |

**Example request**

```bash
curl -X POST "https://api.argalabs.com/validate/a1b2c3d4-e5f6-7890-abcd-ef1234567890/env/discord/_frontend/messages" \
  -H "Authorization: Bearer $ARGA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
The response is the message object created in the Discord twin channel.
...
### `POST` Start a scan (removed)

<Warning>The `/redteam/start` and `/redteam/start-branch` endpoints have been removed. Use [`/validate/agent-run`](#post-start-a-sandbox-run) instead.</Warning>

The `POST /redteam/start` and `POST /redteam/start-branch` endpoints were previously deprecated and have now been removed. Migrate to [`/validate/agent-run`](#post-start-a-sandbox-run), which supports both URL-based and branch-based sandbox runs.
...
An array of scenario objects. Preset scenarios have `is_preset` set to `true` and empty `created_at` / `updated_at` timestamps.
...
[
  {
    ...
    "tags": ["ecommerce", "payments", "functional"],
    "created_at": "",
    "updated_at": "",
    "is_preset": true
  }
...
| `include_presets` | `boolean` | No | When `true`, prepend built-in [preset scenarios](#get-list-preset-scenarios) to the results. Defaults to `false`. |
...
An array of scenario objects (same shape as the [create response](#post-create-a-scenario)), sorted by creation date (newest first). When `include_presets` is `true`, preset scenarios (with `is_preset: true`) appear before user-created scenarios.
...
Update user-facing settings for a connected integration. Currently used to control automatic behavior such as Linear issue creation on failed validation runs.

**Path parameters**

| Parameter | Type | Description |
|-----------|------|-------------|
| `provider` | `string` | Integration provider name (e.g. `linear`). |

**Request body**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `auto_create_linear_issues` | `boolean` | No | When `false`, Arga will not automatically create Linear issues for failed validation runs. Defaults to `true` when the Linear integration is first connected. Only applies to the `linear` provider. |
...
| `metadata` | `object` | Updated integration metadata. Sensitive fields such as tokens are excluded. |
