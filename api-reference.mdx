...
| `/validate/agent-run` | `POST` | — | Yes | Unified agent exploration |
...
| `/redteam/start` | `POST` | — | Yes | **Deprecated** — use `/validate/agent-run` |
| `/redteam/start-branch` | `POST` | — | Yes | **Deprecated** — use `/validate/agent-run` |
...
**Key distinction:** API keys (from `arga login`) are accepted on endpoints designed for programmatic and agent use — starting URL runs, polling run results, and managing twins. JWT tokens (from the web app OAuth flow) are required for interactive features like PR runs, agent runs, and email verification.
...
These endpoints consolidate the previous validation and red-team entry points into three workflows: URL run, PR run, and agent run.
...
Start an autonomous agent exploration run against a deployed URL or a branch. Provide `url` for standalone exploration, or `repo` and `branch` to deploy a branch first and then explore it.
...
<Warning>The `/redteam/start` endpoint is deprecated. Use [`/validate/agent-run`](#post-start-an-agent-run) instead.</Warning>
