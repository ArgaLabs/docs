---
title: "API reference"
description: "HTTP API endpoints for programmatic access to Arga validations, scans, scenarios, and authentication"
---
...
| `/scenarios` | `POST` | Yes | Yes | Create a scenario |
| `/scenarios` | `GET` | Yes | Yes | List scenarios |
...
| `scenario_id` | `string` | No | — | UUID of a [scenario](#scenarios) to automatically seed the provisioned twins with preconfigured data. When provided, the twins are seeded in the background after provisioning completes. |
...
curl -X POST https://api.argalabs.com/validate/twins/provision \
  ...
  -d '{
    "twins": ["slack", "notion"],
    "ttl_minutes": 120,
    "scenario_id": "b2c3d4e5-f6a7-8901-bcde-f12345678901"
  }'
```
...
Scenarios let you define reusable twin seed configurations that pre-populate digital twins with realistic data before a validation run. You can create scenarios from a natural-language prompt (the API uses an LLM to generate the seed configuration) or by providing an explicit seed configuration.
...
Create a new scenario. Provide either a `prompt` (to generate a seed configuration automatically) or a `seed_config` with `twins` (to define the configuration explicitly). If both `prompt` and `seed_config` are provided, the explicit configuration takes precedence.
...
| `description` | `string` | No | Longer description of what this scenario sets up. |
| `prompt` | `string` | No | Natural-language prompt describing the desired twin state. When provided without `seed_config`, the API generates the configuration automatically. |
| `twins` | `array of string` | No | Twin names to include (e.g. `slack`, `stripe`). Required when `seed_config` is provided without `prompt`. |
| `seed_config` | `object` | No | Explicit seed configuration keyed by twin name. Each value is a twin-specific configuration object. |
| `tags` | `array of string` | No | Tags for organizing and filtering scenarios. Defaults to `[]`. |
...
curl -X POST https://api.argalabs.com/scenarios \
  ...
  -d '{
    ...
    "prompt": "Set up Slack with an #orders channel and Stripe with a customer who has an active subscription",
    "tags": ["checkout", "billing"]
  }'
```

**Example request (explicit config)**
...
curl -X POST https://api.argalabs.com/scenarios \
  ...
  -d '{
    ...
    "seed_config": {
      "slack": {
        "channels": [{"name": "support-tickets"}],
        "users": [{"name": "Support Bot"}]
      },
      "notion": {
        "pages": [{"title": "Runbook"}]
      }
...
```json
{
  ...
  "twins": ["slack", "stripe"],
  "seed_config": {
    "slack": {"channels": [{"name": "orders"}]},
    "stripe": {"customers": [{"name": "Test Customer", "subscriptions": [{"status": "active"}]}]}
  },
  "tags": ["checkout", "billing"],
  "created_at": "2026-03-21T12:00:00Z",
...
| Field | Type | Description |
|-------|------|-------------|
| `id` | `string` | Unique identifier for the scenario (UUID). |
| `name` | `string` | Display name. |
| `description` | `string \| null` | Description, if provided. |
| `prompt` | `string \| null` | The natural-language prompt used to generate the seed configuration. |
| `twins` | `array of string` | Twin names included in this scenario. |
| `seed_config` | `object` | Seed configuration keyed by twin name. |
...
|--------|--------|
| `401` | Invalid or expired token. |
| `422` | Seed configuration generation failed. Returned when LLM-based generation from a prompt does not produce a valid configuration. |
...
List all scenarios owned by the authenticated user, ordered by creation date (newest first).

**Query parameters**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `twin` | `string` | No | Filter to scenarios that include this twin (e.g. `slack`). |
| `tag` | `string` | No | Filter to scenarios that include this tag. |

**Example request**

```bash
curl "https://api.argalabs.com/scenarios?twin=slack" \
  -H "Authorization: Bearer $ARGA_API_KEY"
```
...
Returns a scenario object (same shape as [Create a scenario](#post-create-a-scenario) response).
...
Update an existing scenario. All fields are optional — only provided fields are updated. If `prompt` is updated without a new `seed_config`, the seed configuration is regenerated automatically.
...
| `prompt` | `string` | No | Updated natural-language prompt. If changed and no `seed_config` is provided, the configuration is regenerated. |
...
```bash
curl -X PUT https://api.argalabs.com/scenarios/b2c3d4e5-f6a7-8901-bcde-f12345678901 \
  -H "Authorization: Bearer $ARGA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "E-commerce checkout v2",
    "tags": ["checkout", "billing", "v2"]
  }'
```

**Response**

Returns the updated scenario object (same shape as [Create a scenario](#post-create-a-scenario) response).
...
| `401` | Invalid or expired token. |
| `404` | Scenario not found. |
| `422` | Seed configuration generation failed. |
...
Permanently delete a scenario. Validation runs that reference this scenario are not affected — the `scenario_id` on those runs becomes a dangling reference.
