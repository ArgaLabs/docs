...
const allScenarios = await client.scenarios.list({ includePresets: true });
```
...
const presets = await client.scenarios.listPresets();

// Filter presets by twin or tag
const stripePresets = await client.scenarios.listPresets({ twin: 'stripe' });
const paymentPresets = await client.scenarios.listPresets({ tag: 'payments' });
```
