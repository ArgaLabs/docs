...
<AccordionGroup>
  ...
  <Accordion title="GitHub" icon="github">
    ...
    **Intercepts:** `api.github.com`, `github.com`

    **Quickstart state:**

    - Organization: "test-org" ("Test Organization")
    - Repository: `test-org/demo-repo` (Python, public)
    - User: `testuser` ("Test User")
    - Default branch: `main`
    - Authentication: `Bearer` or `token` header (any well-formed value accepted)

    **Supported resources:** Repos, branches, commits, pull requests, issues, contents, check runs, search, orgs, users, webhooks, and GitHub App endpoints.

    **OAuth support:** The GitHub twin emulates the OAuth authorization code flow so you can test GitHub App install flows without hitting real GitHub servers.

    **Browser UI:** A dark-themed dashboard lets you browse repos, branches, PRs, files, and user profiles backed by the twin's in-memory state.
  </Accordion>
...
| GitHub | `GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_API_URL` |
